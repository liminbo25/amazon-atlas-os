...
export function DiagnosticsWorkbench() {
  ...
  async function submitAnalysis(): Promise<void> {
    ...
    if (!ASIN_PATTERN.test(normalizedTargetAsin)) {
      failAnalysis("目标 ASIN 必须是有效的 10 位 ASIN。", "targetAsin_invalid");
      return;
    }
    ...
    if (invalidCompetitorAsins.length > 0) {
      failAnalysis(
        `竞品 ASIN 必须是有效的 10 位 ASIN。无效值：${invalidCompetitorAsins.join(", ")}。`,
    ...
    if (duplicateCompetitorAsins.length > 0) {
      failAnalysis(
        `竞品 ASIN 不能重复。重复值：${Array.from(new Set(duplicateCompetitorAsins)).join(", ")}。`,
    ...
    if (spApiConfig.mode === "runtime") {
      ...
      if (missingFields.length > 0) {
        failAnalysis(
          `SP-API 运行时模式需要填写 ${missingFields.map(formatSpApiFieldLabel).join(", ")}。`,
    ...
    try {
      ...
      const normalizedError = normalizeApiRequestError(error, "Listing 诊断失败。");
