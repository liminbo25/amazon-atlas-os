...
const LEGACY_DIAGNOSIS_SYSTEM_PROMPT = [
  ...
  "Think like a senior Amazon marketplace operator, not a generic copywriter.",
  "Tie every recommendation to evidence from keyword gaps, review themes, competitor signals, current copy coverage, or score pillars.",
  "Prioritize actions by expected ranking impact, conversion impact, and implementation urgency.",
...
export type LegacyCopyDiagnosisProgressPhase =
  | "collecting_seller_sprite"
  | "building_rule_diagnosis"
  | "generating_ai_recommendations"
  | "finalizing";

export interface LegacyCopyDiagnosisProgress {
  phase: LegacyCopyDiagnosisProgressPhase;
  phaseLabel: string;
  progress: number;
}

export type LegacyCopyDiagnosisProgressReporter = (
  progress: LegacyCopyDiagnosisProgress
) => Promise<void> | void;

export async function runLegacyCopyDiagnosisRequest(
  body: Record<string, unknown>,
  request?: Request,
  reportProgress: LegacyCopyDiagnosisProgressReporter = () => undefined
): Promise<LegacyDiagnosisReport> {
  ...
  await reportProgress({
    phase: "building_rule_diagnosis",
    phaseLabel: "Scoring the listing against keyword, review, and competitor evidence",
    progress: 50,
  });
  ...
  await reportProgress({
    phase: "generating_ai_recommendations",
    phaseLabel: "Generating expert AI recommendations from the full diagnosis",
    progress: 70,
  });
  ...
  await reportProgress({
    phase: "finalizing",
    phaseLabel: "Finalizing the diagnosis report",
    progress: 95,
  });
...
export function normalizeLegacyCopyDiagnosisError(error: unknown): unknown {
