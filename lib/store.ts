...
const initialState = {
  ...
  valuePoints: [],
  competitorAnalysis: [],
  vocActionPlan: null as import("./types").VocActionPlan | null,
  supportFaqs: [] as import("./types").SupportFaqItem[],
  listingVersions: [],
  complianceResults: {},
  isLoading: false,
};
...
export const useListingStore = create<ListingStore>()(
  persist(
    (set, get) => ({
      ...
      setVocActionPlan: (plan) => set({ vocActionPlan: plan }),
      setSupportFaqs: (items) => set({ supportFaqs: items }),
